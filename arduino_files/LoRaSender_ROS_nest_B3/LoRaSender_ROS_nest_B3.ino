...
const int syncWord = 0x36;
byte localAddress = 0xB3;     // address of this device
byte destination = 0xA3;      // destination to send to
...
void messageCb3( const std_msgs::String correction_data_msg_B3)
{
  digitalWrite(blue_pin, HIGH);
  unsigned long currentMicros1 = micros();
  // digitalWrite(orange_pin, HIGH-digitalRead(orange_pin));   // toggle the led
  correction_data = correction_data_msg_B3.data; // String
...
ros::Subscriber<std_msgs::String> sub3("correction_data_msg_B3", &messageCb3 );
